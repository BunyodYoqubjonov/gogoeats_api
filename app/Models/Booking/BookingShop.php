 ...
 * @property integer $customer_delivery_fee
 * @property integer $business_delivery_fee
 * @property integer $rate_customer_delivery_fee
 * @property integer $rate_business_delivery_fee
...
//        'visibility'    => 'boolean',
    ...
    public function getRateCustomerDeliveryFeeAttribute(): ?float
    {
        if (request()->is('api/v1/dashboard/user/*') || request()->is('api/v1/rest/*')) {
            return $this->customer_delivery_fee * $this->currency();
        }

        return $this->customer_delivery_fee;
    }

    public function getRateBusinessDeliveryFeeAttribute(): ?float
    {
        if (request()->is('dashboard/user') || request()->is('api/v1/rest/*')) {
            return $this->business_delivery_fee * $this->currency();
        }

        return $this->business_delivery_fee;
    }
			...
			DeliveryZone::list()
				...
				->map(function (DeliveryZone $deliveryZone) use ($filter, &$orders) {
					...
					if (
						...
						$orders[$deliveryZone->shop_id] = $deliveryZone->customer_delivery_fee;
					}
