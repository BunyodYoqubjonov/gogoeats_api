...
class OrderResource extends JsonResource
{
    ...
    public function toArray($request): array
    {
        ...
        $isUser = (request()->is('api/v1/rest/*') || request()->is('api/v1/dashboard/user/*'));

		$deliveryFee = $this->rate_delivery_fee + (!$isUser ? $this->rate_business_delivery_fee : 0);
		$smallOrderFee = $this->rate_small_order_fee;

		if ($this->rate_business_delivery_fee <= 0 && !$isUser) {
			$deliveryFee += $this->small_order_fee;
			$smallOrderFee = 0;
		}
        ...
        return [
            ...
            'origin_price'                  => $this->when($this->rate_total_price, $this->rate_total_price + $this->rate_total_discount - $this->rate_tax - $deliveryFee - $this->rate_service_fee - $smallOrderFee + $couponPrice),
            'seller_fee'                    => $this->when($this->rate_total_price, $this->rate_total_price - $deliveryFee - $this->rate_service_fee - $this->rate_commission_fee - $this->rate_coupon_sum_price - $this->point_histories_sum_price - $smallOrderFee),
            ...
            'small_order_fee'               => $this->when($smallOrderFee, $smallOrderFee),
            ...
            'business_delivery_fee'         => $this->when($this->rate_business_delivery_fee, $this->rate_business_delivery_fee),
            ...
            'delivered_at'                  => $this->when($this->delivered_at, "{$this->delivered_at}Z"),
