...
class MaksekeskusService extends BaseService
{
    ...
    public function subscriptionProcessTransaction(array $data, Shop $shop, $currency): Model|PaymentProcess
    {
        ...
        $body = [
            ...
            'return_url' => "$host/payment-success?subscription_id=$subscription->id&lang=$this->language&status=success",
            'cancel_url' => "$host/payment-success?subscription_id=$subscription->id&lang=$this->language&status=canceled",
            'notification_url' => "$host/api/v1/webhook/maksekeskus/payment",
            'transaction_url' => [
                'return_url' => [
                    'url'    => "$host/payment-success?subscription_id=$subscription->id&lang=$this->language&status=success",
                    'method' => 'POST',
                ],
                'cancel_url' => [
                    'url'    => "$host/payment-success?subscription_id=$subscription->id&lang=$this->language&status=canceled",
                    'method' => 'POST',
                ],
                'notification_url' => [
                    'url'    => "$host/api/v1/webhook/maksekeskus/payment",
