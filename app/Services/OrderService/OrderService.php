...
class OrderService extends CoreService implements OrderServiceInterface
{
    ...
    public function create(array $data): array
    {
        ...
        /** @var Shop $shop */
		$shop = Shop::with(['deliveryZones'])->find(data_get($data, 'shop_id'));

		$deliveryZone = null;

		if (data_get($data, 'delivery_type') === Order::DELIVERY) {

			$result = Utility::checkMultiPolygon($data, $shop, $this->language);
...
//			}
			...
			/** @var Order $order */
			$order = $this->model()->updateOrCreate($this->setOrderParams($data));
			...
			$this->calculateOrder($order, $shop, $data, $deliveryZone);
    ...
    public function update(int $id, array $data): array
    {
        try {
            ...
            $order = DB::transaction(function () use ($data, $id) {
                ...
                /** @var Shop $shop */
                $shop = Shop::with(['seller', 'deliveryZones'])->find(data_get($data, 'shop_id'));
				...
				if (data_get($data, 'delivery_type') === Order::DELIVERY) {
					...
					if (!data_get($result, 'status')) {
						throw new Exception(data_get($result, 'message', 'error in update'));
					}
	 ...
	 * @param array $data
	 * @param DeliveryZone|Collection|null $deliveryZone
	 * @return void
	 * @throws Exception
	 */
    private function calculateOrder(Order $order, ?Shop $shop, array $data, DeliveryZone|Collection|null $deliveryZone): void
    {
        /** @var Order $order */
        ...
        $totalPrice += ($order->delivery_fee + $totalDiscount);
        ...
        $deliveryFee = max($deliveryZone?->customer_delivery_fee, 0);
        ...
        $order->update([
            ...
            'business_delivery_fee' => $deliveryZone?->business_delivery_fee,
