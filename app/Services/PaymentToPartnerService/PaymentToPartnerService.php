...
class PaymentToPartnerService extends CoreService
{
    ...
    {
		$deliveryFee = $order->delivery_fee + $order->business_delivery_fee;

		if ($order->business_delivery_fee <= 0) {
			$deliveryFee += $order->small_order_fee;
		}
	...
	private function addForDeliveryman(Order $order, User $deliveryman, Payment $payment): void
    {
		...
		if ($payment->tag === 'wallet') {
			...
			(new WalletHistoryService)->create([
				'type'  	=> $deliveryFee ? 'topup' : 'withdraw',
				'price' 	=> (double)str_replace('-', '', $deliveryFee),
			...
			(new WalletHistoryService)->create([
				'type'  	=> $deliveryFee ? 'withdraw' : 'topup',
				'price' 	=> (double)str_replace('-', '', $deliveryFee),
